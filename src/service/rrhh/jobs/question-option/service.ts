...
export function useQuestionOptions(id: string) {
  ...
  export async function createQuestionOptions(id:string, data: any, handler: () => void, update: () => void) {
    try{
        if(data.type === "select"){
        ...
        if(response.status === 201){
            data.lengt > 1 ? toast.success("Opciones creadas correctamente", {
                duration: 4000,
                position: 'top-center',
        ...
        data.lengt > 1 ? toast.error("Error al crear las opciones", {
            duration: 4000,
            position: 'top-center',
  ...
  export async function editQuestionOptions(id: string, data: any, handler: () => void, update: () => void){
    try {
        const url = `${process.env.NEXT_PUBLIC_API_URL}/question-option/${id}`;
  ...
  export async function deleteQuestionOption(id: string, handler: () => void, update: () => void){
    try {
        const url = `${process.env.NEXT_PUBLIC_API_URL}/question-option/${id}`;
