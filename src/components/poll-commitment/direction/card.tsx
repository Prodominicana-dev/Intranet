import {
  ...
  import { deleteDirection} from "@/service/rrhh/poll-commitment/direction/service";
  ...
  export default function Card({
    ...
    const handleDelete = () => {
      //if (user && !isLoading) {
        deleteDirection(direction.id as string, handleDeleteOpen, update);
      //}
    };
    return (
      <>
        <div className="grid items-center m-1 w-full py-4 grid-cols-2 text-center bg-white rounded-lg  ring-2 ring-gray-100">
          <div className="line-clamp-1">
            <Tooltip content={direction?.name}>{direction?.name}</Tooltip>
          </div>
