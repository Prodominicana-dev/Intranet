...
import { createQuestionOptions, editQuestionOptions } from "@/service/rrhh/jobs/question-option/service";


export function EditQuestionOptionDialog({
    option,
  question,  
  open,
  handler,
  ...
  const handleSubmit = async () => {
    ...
    await editQuestionOptions(option.id, data, handler, update);
    setIsLoading(false);
  }
  ...
  return (
    <>
      ...
      >
        ...
        >
                <div><div className='w-full text-black font-bold text-xl'>{question.question}</div>
                <div className='w-full text-black text-sm'>Pregunta</div></div>
          <form  className="w-full flex flex-col gap-5" action={handleSubmit}>
            <label htmlFor="question" className="text-black font-2xl font-bold">Opción <span className="text-red-600">*</span></label>
              <input value={options} onChange={(e) => setOptions(e.target.value)} id="options" className="py-2 px-2 text-black rounded-lg ring-1 ring-gray-400"></input>
          </form>
        </DialogBody>
        <DialogFooter
         onPointerEnterCapture={undefined}
